...
Adafruit_GPS GPS(&mySerial); // the gps object will load from serial 1
...
const int SDpinCS         = 53; // sd card CS  
const int pinRecordBtn    = 7;  // button for recording
const int ledPower        = 13; //6;
const int ledRecording    = 5;  // led to show is recording
const int ledGPSconnected = 4;  // not used yet
...
//      strcat(s,SerialInputReady);
      strcat(s,(int)GPS.satellites));
      CB.ConfirmAdd(s);
...
void loop()
{
  // Every thing is in a test for any issues
  if(!isIssue) // avoid any work if issue detected, power down needed
  {
  //---------------------------------------------------------
  // ---- Handle recording button status has just changed ---
  //---------------------------------------------------------
    i = digitalRead(pinRecordBtn);
    if (i == HIGH) // button is on
    {
      // if we've just started recording
      // - mark isRecording as true
      // - switch LED on
      // - Create New File
      if(!isRecording)      // Have we just started recording
      {
        isRecording = true; // update flag to reflect we are recording
        CreateFileName();
  ...
  //---------------------------------------------------------
  // ---- Write out data if recording ---
  //---------------------------------------------------------
    if (isRecording) // do it only once 
    {
      if(CB.AnythingToGet() > 10) 
      {
        ...
        if(f)
        {
          ...
          {
            if(GPS.fix) // this is from the GPS library
            {
              f.println(s);
              entries++;
            }
  ...
  { // flash the power light when there is an issue
    if(isPower)
