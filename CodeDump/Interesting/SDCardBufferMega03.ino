...
boolean TCardBuffer::StringGet(String *s)
{
  ...
  if(lRead < lWrote){ // there is something to read
    ...
    *s = "For Debugging: Cycle.." + String(CycleLastRead) +
      ...
      " and Value: ," + sBuf[ElementLastRead];
    Serial.println(*s);
  }
  else
  {
...
//    }
    ...
    if(CB.AnythingToGet())
    {
      ...
      if(f)
      {
        Serial.println("..Writing to file.. number of entries should be around 60 by the timer");
