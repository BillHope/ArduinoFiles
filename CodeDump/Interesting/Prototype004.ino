...
//
  if(!isIssue) // avoid any work if issue detected, power down needed
  {
    ...
    if (i == HIGH) // button is down
    {
       if(!isRecBtnDown) // button wasn't pressed before
      {
        isRecording = !isRecording; // toggle the mode
        isRecBtnDown = true;        // flag we got that button press
        if(isRecording)  // we are now recording
        {                
          CreateFileName();
          digitalWrite(ledRecording,HIGH);
        }
        else digitalWrite(ledRecording,LOW);
      }
    } else isRecBtnDown = false;    // Recording button is not being pressed
